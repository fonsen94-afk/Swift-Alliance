...
# --- Simple in-file "bank" backend (minimal) ---------------------------------------
...
def create_demo_customer_and_accounts():
    # Create one demo customer and two accounts if none exist
...
def build_mt_message(msg_type: str, fields: Dict[str, str], sender_info: Dict[str,str], reference: Optional[str]=None) -> str:
    ...
    for k, v in fields.items():
        lines.append(f"{k}{v}")
    # Append sender details as narrative if present
...
# --- ISO20022 pain.001 generator (simple) ----------------------------------------
...
def download_logo_from_url(url: str) -> Optional[str]:
    """Download logo and save to assets/logo.*; returns path or None"""
    try:
...
def build_formal_output(message_type: str, message_body: str, sender_info: Dict[str,str], start_ts: str, end_ts: str, account_number: str) -> str:
    """Create the formal textual output used for TXT and for rendering in PDF."""
    header_lines = [
...
def generate_pdf_bytes(formal_text: str, logo_path: Optional[str]=None) -> bytes:
    ...
    # Split formal_text into lines and render with wrapping
...
# Sender info (prefill with provided details)
...
# MT-specific field builder (simple)
st.markdown("MT Fields (key -> value). For example tag ':59:' -> beneficiary details.")
mt_fields_raw = st.text_area("Enter MT tag lines (one per line) like ':59:Beneficiary /IBAN...'", height=120)
mt_fields = {}
...
if start_btn:
    ...
    dos_lines = [
        ...
        "",
        "Initializing SWIFT message composer...",
        "Loading modules: [OK]",
        "Establishing secure environment: [OK]",
        "Ready. Press Compose to build the message.",
    ]
    show_dos_boot(dos_lines, delay=0.18)
...
if compose_btn:
    ...
    if msg_type == "MT103":
        ...
        try:
            xml = generate_pain001_xml(payment)
            message_body = xml
        except Exception as e:
...
if download_pdf_btn:
    if not st.session_state.get("formal_text") and not st.session_state.get("preview"):
        ...
        try:
            ...
            st.error(f"PDF generation failed: {e}. Install reportlab.")
...
st.markdown("----")
st.caption("This app generates SWIFT-style messages for internal use and export. It does not connect to the SWIFT network.")
